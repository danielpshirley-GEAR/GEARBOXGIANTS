  ...
  static renderReport(container, vehicle, diagnostic, customer) {
    ...
    container.innerHTML = `
      <div class="report-wrapper animate-fade-in">
        ...
        <!-- Customer Delivery Notification Banner (Structured Hierarchy) -->
        <div class="report-delivery-banner">
          <div class="delivery-text" style="flex:1 1 480px; min-width:0;">
            <!-- Level 1: Status & Timestamp Meta -->
            <div style="display:flex; align-items:center; gap:0.6rem; flex-wrap:wrap; margin-bottom:0.4rem;">
              <span style="background:rgba(34,197,94,0.18); color:#4ade80; border:1px solid rgba(34,197,94,0.35); font-size:0.72rem; font-weight:800; padding:2px 8px; border-radius:9999px; text-transform:uppercase; letter-spacing:0.04em;">
                Report Generated
              </span>
              <span style="color:#94a3b8; font-size:0.78rem; font-weight:700;">
                Ref: <strong style="color:#ffffff; font-family:var(--font-mono, monospace);">${refNumber}</strong>
              </span>
            ...
            <h3 style="font-family:var(--font-heading), sans-serif; font-size:1.25rem; font-weight:900; color:#ffffff; margin:0 0 0.35rem 0; letter-spacing:0.01em;">
              Diagnostic Report for ${customer.name || 'Vehicle Owner'}
            </h3>

            <!-- Level 3: Dispatch Destination -->
            <div style="font-size:0.84rem; color:#94a3b8; display:flex; flex-wrap:wrap; align-items:center; gap:0.35rem 0.6rem;">
              <span>Dispatched to <strong style="color:#cbd5e1;">${customer.email}</strong></span>
              ${customer.phone ? `
                <span style="color:rgba(255,255,255,0.2);">&bull;</span>
                <span>SMS to <strong style="color:#cbd5e1;">${customer.phone}</strong></span>
              ` : ''}
            </div>
          </div>

          <!-- Actions -->
          <div class="delivery-actions" style="display:flex; gap:0.6rem; align-items:center; flex-wrap:wrap;">
            <button type="button" class="btn btn-secondary btn-sm" id="btn-print-report" style="padding:0.5rem 1rem; font-size:0.82rem; font-weight:800; border-radius:6px;">
              Print / Save PDF
            </button>
            <button type="button" class="btn btn-outline btn-sm" id="btn-new-diagnostic" style="padding:0.5rem 1rem; font-size:0.82rem; font-weight:800; border-radius:6px;">
              Retest Vehicle
            </button>
          </div>
        </div>
    ...
    if (btnClaimQuote) {
      btnClaimQuote.addEventListener('click', () => {
        if (window.openQuoteModal) {
          ...
          const emailField = document.getElementById('quote-email') || document.getElementById('modal-email');
          if (emailField && customer.email) emailField.value = customer.email;

          const postcodeField = document.getElementById('quote-postcode');
          if (postcodeField && customer.postcode) postcodeField.value = customer.postcode;

          const notesField = document.getElementById('quote-desc') || document.getElementById('modal-notes') || document.querySelector('textarea[name="notes"]');
          if (notesField) {
            notesField.value = `[DIAGNOSTIC REPORT SUMMARY]\nVehicle: ${vehicle.year} ${vehicle.make} ${vehicle.model} (${vehicle.registration})\nGearbox: ${vehicle.gearboxFamily}\nIdentified Fault: ${diagnostic.findings.map(f => f.title).join('; ')}\nHealth Score: ${diagnostic.healthScore}/100 (${diagnostic.urgencyLevel})\nCustomer: ${customer.name || ''} (${customer.email} / ${customer.phone || 'No phone'})`;
          }
        }
      });
    }
