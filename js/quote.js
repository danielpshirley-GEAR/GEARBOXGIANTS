...
(function () {
  ...
  function parseUkRegistration(rawPlate) {
    ...
    // Standard UK Plate Format: 2 letters, 2 digits, 3 letters (e.g. RO74MSX, EA19XPR)
                ...
                <div style="display:grid; grid-template-columns:1.3fr 0.7fr; gap:0.75rem;">
                  <div class="form-group" style="margin-bottom:0;">
                    ...
                    <select id="quote-transmission-select" class="form-select" onchange="window.handleTransSelectChange(this.value, 'modal', true)" style="padding:0.65rem 0.85rem; font-size:0.88rem;">
                      <option value="Automatic (ZF / Torque Converter / Steptronic)">Automatic (ZF / Steptronic)</option>
                      <option value="Dual-Clutch (DSG / S-Tronic / DCT / Powershift / PDK)">Dual-Clutch (DSG / S-Tronic)</option>
                      <option value="Manual (5-Speed / 6-Speed Transmission)">Manual Gearbox</option>
                      <option value="CVT / e-CVT Continuously Variable">CVT / Hybrid</option>
                      <option value="Semi-Automatic / Mechatronics Service">Semi-Auto / Mechatronics</option>
                      <option value="Electric Drive Unit">Electric Drive Unit</option>
                    </select>
                  </div>
                  <div class="form-group" style="margin-bottom:0;">
  ...
  function renderVehicleResult(vehicle, cleanPlate, scope, isLiveDvla) {
    ...
    resultElem.innerHTML = `
      <div class="diag-fade-in" style="background:rgba(16,185,129,0.06); border:1px solid rgba(16,185,129,0.3); border-radius:8px; padding:0.85rem 1rem; margin-top:0.75rem; text-align:left;">
        <div style="display:flex; align-items:center; justify-content:space-between; gap:0.5rem; flex-wrap:wrap; margin-bottom:0.35rem;">
          <div style="display:flex; align-items:center; gap:0.45rem;">
            <span style="font-size:1.05rem; font-weight:800; color:#fff; font-family:var(--font-heading);">${vehicle.year || ''} ${vehicle.make}</span>
          </div>
          <div style="display:flex; align-items:center; gap:0.75rem;">
            <button type="button" onclick="window.focusVehicleSelect('${scope}')" style="background:none; border:none; color:var(--text-muted); font-weight:600; cursor:pointer; text-decoration:underline; font-size:0.75rem; padding:0;">
              Adjust Spec ✏️
            </button>
        ...
        <div style="font-size:0.92rem; color:var(--amber-400); font-weight:800; margin-bottom:0.45rem;">
          ${vehicle.model || vehicle.make}
        </div>
        <div style="display:flex; flex-wrap:wrap; gap:0.4rem; font-size:0.76rem;">
          <span style="background:rgba(255,255,255,0.06); border:1px solid rgba(255,255,255,0.08); padding:3px 8px; border-radius:4px; color:#cbd5e1;">${vehicle.transmission || 'Automatic / DSG'}</span>
          <span style="background:rgba(255,255,255,0.06); border:1px solid rgba(255,255,255,0.08); padding:3px 8px; border-radius:4px; color:#cbd5e1;">${vehicle.engine || 'Standard'}</span>
          ${vehicle.mileage ? `<span style="background:rgba(255,255,255,0.06); border:1px solid rgba(255,255,255,0.08); padding:3px 8px; border-radius:4px; color:#cbd5e1;">${vehicle.mileage}</span>` : ''}
          ${vehicle.motStatus ? `<span style="background:rgba(255,255,255,0.06); border:1px solid rgba(255,255,255,0.08); padding:3px 8px; border-radius:4px; color:#cbd5e1;">MOT: ${vehicle.motStatus}</span>` : ''}
        </div>
      </div>
    `;
  }
  ...
  function syncVehicleToForm(vehicle, scope = 'modal') {
    ...
    if (transSelect) {
      ...
      for (let i = 0; i < transSelect.options.length; i++) {
        const optText = transSelect.options[i].text.toLowerCase();
        if ((transLower.includes('dual-clutch') || transLower.includes('dsg') || transLower.includes('s-tronic') || transLower.includes('dct') || transLower.includes('7g-dct') || transLower.includes('powershift') || transLower.includes('pdk')) && optText.includes('dual-clutch')) {
          transSelect.selectedIndex = i; break;
        } else if ((transLower.includes('zf') || transLower.includes('8hp') || transLower.includes('6hp') || transLower.includes('steptronic') || transLower.includes('eat8') || transLower.includes('torque converter') || transLower.includes('automatic')) && optText.includes('automatic')) {
          transSelect.selectedIndex = i; break;
        } else if (transLower.includes('manual') && optText.includes('manual')) {
          transSelect.selectedIndex = i; break;
        } else if ((transLower.includes('cvt') || transLower.includes('e-cvt') || transLower.includes('hybrid')) && optText.includes('cvt')) {
          transSelect.selectedIndex = i; break;
        } else if ((transLower.includes('semi-automatic') || transLower.includes('mechatronic')) && optText.includes('mechatronics')) {
          transSelect.selectedIndex = i; break;
        } else if (transLower.includes('electric') && optText.includes('electric')) {
          transSelect.selectedIndex = i; break;
        }
      }
    }
